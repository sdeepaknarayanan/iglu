...
train_labels = np.zeros((train_laplacian.shape[0], 121))
train_labels[train_ix] = labels[train_ix]
train_labels = train_labels.astype(np.float32)

valid_labels = np.zeros((train_laplacian.shape[0], 121))
valid_labels[valid_ix] = labels[valid_ix]
valid_labels = valid_labels.astype(np.float32)

test_labels = np.zeros((train_laplacian.shape[0], 121))
...
R1 = glorot([1024, 512], name = "R1")
R2 = glorot([1024, 512], name = "R2")

w = glorot([1024, 121], name = "w")

X0 = tf.Variable(np.zeros((full_laplacian.shape[0], 50)).astype(np.float32), trainable = False)
X1 = tf.Variable(np.zeros((full_laplacian.shape[0], 1024)).astype(np.float32), trainable = False)
X2 = tf.Variable(np.zeros((full_laplacian.shape[0], 1024)).astype(np.float32), trainable = False)
alpha_2_var = tf.Variable(np.zeros((full_laplacian.shape[0], 1024)).astype(np.float32), trainable = False)
...
train_loss = masked_sigmoid_cross_entropy(preds, train_labels, train_mask) 
...
for epoch in range(300):
    ...
    valid_f1 = calc_f1(labels[valid_ix], valid_preds, True)[0]
    test_f1 = calc_f1(labels[test_ix], test_preds, True)[0]
    train_f1 = calc_f1(labels[train_ix], train_preds, True)[0]
